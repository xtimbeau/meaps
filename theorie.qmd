...
Nous développons ici les principaux éléments théoriques de MEAPS.
L'utilisation du modèle gravitaire pour quantifier le nombre de déplacements entre une origine et une destination souffre de plusieurs problèmes.
L'analogie avec la gravitation est intuitive, mais permet mal de comprendre le phénomène microscopique à l'œuvre.
Il en découle que le respect de contraintes en ligne est surtout en colonne est réalisé de façon *ad hoc* sans réelle maîtrise des hypothèses sous-jacentes.
Par ailleurs, en donnant une même importance à la distance quelque soit la densité du milieu en opportunité, le modèle gravitaire tend à prédire des motifs de déplacements très limités lorsque les opportunités sont spatialement rares.

Pour palier à ces défauts, nous utilisons une analogie différente, empruntée elle aussi à la physique.
Il s'agit du modèle radiatif qui décrit l'absorption d'une particule dans un milieu dont la densité est hétérogène.
Ce modèle a été énoncé dans une forme préliminaire par @stouffer1940 et développé en suivant l'analogie avec la physique par @simini2012.
En simplifiant le formalisme de @simini2012, nous pouvons ajouter deux éléments importants :

-   une saturation du l'absorption lorsque l'opportunité est choisie par suffisamment d'individus.
    Pour que cela ait un sens, les individus ont un ordre de priorité, arbitraire dans leurs choix d'opportunité.

-   un raisonnement statistique pour annuler l'effet de l'ordre des choix et de la saturation.
    En prenant la moyenne sur tous les ordres de priorité, on peut s'abstraire de chaque ordre et construire une mesure ergodique de la saturation.
...
La procédure de respect des marges peut être formulée différemment, par exemple en utilisant des corrections additives au lieu de corrections multiplicatives ou une combinaison d'additivité et de multiplicativité.
Chacune de ces procédures manque autant de fondements théoriques que la procédure habituelle et, en toute généralité, peuvent conduire à des équilibres multiples que l'algorithme de résolution va sélectionner sans que l'on puisse justifier quoi que ce soit.
...
Le modélisateur peu s'appuyer une analogie simple et confirme à l'intuition tout en disposant de quelques leviers pour se caler sur les données.
...
L'estimation ainsi conduite est non paramétrique et permet de construire une transformation des observations comme la déviation par rapport à un schéma de référence.
Nous explorons également une détermination parcimonieuse des coefficients correcteurs afin de pouvoir extraire une information pertinente des données de flux entre communes et de pouvoir comparer le pouvoir prédictif de MEAPS à celui d'un modèle gravitaire, à degré de liberté égal.
...
### Synthèse des aspects théoriques de MEAPS
