...
access_4modes <- 
  map(set_names(c("to1k", "to5k", "to10k", "to20k")), ~{ 
    ggplot()+
      decor_carte +
...
names(nacc) <- nacc |> str_remove("to") |> str_remove("k") |> as.numeric() |> format(big.mark=" ")
src <- lapply(names(access_4modes), function(x) knit_expand(file = "access_template.qmd"))
  
  
#   
#   size <- format(1000*(as.numeric(str_remove(.y, "to") |> str_remove("k"))), big.mark = " ")
#   
#   cat(glue("#### temps pour {str_remove(.y, 'to')}"), "\n")
#   
#   # cat(glue("'Temps d'accès à {size} emplois. Pour chaque carreau de résidence, on détermine le temps minimal pour atteindre au moins {size} emplois suivant l'un des quatre modes considéré. Cette mesure remplace la distance dans le modèle radiatif. <br>Calcul des auteurs. <br>Source : OSM, Mapbox, IGN, Conveyal R5, carroyage INSEE 2017, Flores et fichiers fonciers 2018'"), "\n")
#   # cat("#| fig-asp: 0.9", "\n")
#   print(.x)
#   cat(str_c("{", glue("#fig-4acc{.y}"), "}"))
#   cat("\n\n")
# })
