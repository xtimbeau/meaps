...
### $R^2_{KL}$, ergodicité et émiettage
...
Dans les simulations synthétiques présentées dans le @sec-synth les flux sont simulés avec une granularité unitaire. Chaque emploi ou chaque individu y est localisé et les distances sont calculées entre ces localisations. L'agrégation spatiale à la maille hexagonale se fait ensuite. Dansle cas des données que nous utilisons pour La Rochelle, les carreaux ne sont pas occupés par un seul résident actif ou un seul emploi. Il y a des paquets pour lesquels il n'est pas nécessaire de refaire les simulations individu par individu ou emploi par emploi. Nous les avons donc regroupés et simuler en conséquences dans MEAPS. Cela pose cependant un problème puisque le choix d'un ordre de priorité s'exerce maintenant sur des individus en paquets de taille différente, un faible nombre de ces paquets étant de taille très supérieure à la médiane des autres. Ainsi, lorsqu'un paquet de taille importante est à son tour de choisir, il peut saturer des emplois en une seule passe. Pour résoudre ce problème, nous procédons à un émiettage dans lesquels les paquets de taille importante sont divisés en paquets plus petits de taille inférieure à un seuil. Pour un seuil de 10 individus, on augmente le nombre de paquets d'environ 50% ce qui permet de conserver un problème de taille globale raisonnable tout en réduisant le problème de granularité des paquets. De plus, les paquets sont tirés au sort dans leur ordre de priorité en tenant compte de leur taille afin d'éviter une sur-représentation des paquets de petite taille dans les priorités.
...
Ceci n'est autre que l'entropie de la distribution $p$ à une constante près ($N$ est le nombre de résidents actifs ou d'emplois).

La @fig-reflr représente le $R^2_{KL}$ que l'on calcule pour le modèle de référence en effectuant des simulations de Monte-Carlo pour différentes tailles d'échantillon d'ordre. Sans surprise, plus l'échantillon est grand, plus la distribution des $R^2_{KL}$ est étroite. Pour 256 tirages, l'intervalle de confiance à 95% pour le $R^2_{KL}$ est de l'ordre de 0.017% (contre 0.04% pour 64 tirages et 0.003% pour 1024 tirages) ce qui sera suffisant pour la plupart des applications.
...
ggplot(density)+
  geom_area(aes(
    ...
    alpha=0.95, position="identity")+
  scale_x_continuous(labels = scales::label_percent(.01))+
  scale_color_brewer(palette="Blues", name = "Tirages M.C." , aesthetics = c("color", "fill"))+
  xlab(latex2exp::TeX("$R^2_{KL}$"))+
  theme_ofce()
```

### Estimation de la première approche {#sec-estnp}
