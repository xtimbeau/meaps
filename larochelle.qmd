...
$$
msre = \Sigma_i({log(\hat t}_i) - log(t_i))^2
$$
...
$$
msre = \Sigma_i { t_i \times ({log(\hat t}_i) - log(t_i))^2 }
$$
...
4.  les paramètres du modèle gravitaire sont difficilement réconciliables avec l'interprétation. Ils dépendent en théorie de la configuration spatiale et de l'échelle d'observation. MEAPS permet une approche structurelle aux fondements bien définis qui donne aux paramètres une signification directement interprétable.
...
Le tableau donne les résultats de l'estimation d'un modèle gravitaire non contraint (i.e $\alpha$ et $\beta$ sont estimés).

```{r}
#| label: tbl-gravcom
#| tbl-cap: Estimation commune à commune de modèles gravitaires non contraints, métrique standard
library(gtsummary)
