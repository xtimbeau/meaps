...
Avant de produire une application complète pour l'agglomération de La Rochelle (dans un prochain document), nous testons ici le modèle MEAPS pour l'analyse des mobilités en le comparant en particulier au modèle gravitaire. La question est de savoir quel modèle est le mieux à même de reproduire les données observées de flux, à savoir MOBPRO en introduisant un minimum de paramètres.
...
[@josselin2020] utilisent une forme similaire, avec quelques variations, pour estimer dans la région PACA, à partir des données MOBPRO, des modèles gravitaires. Cette forme et la stratégie d'estimation posent plusieurs problèmes.

Le premier est que lorsque $\alpha$ et $\beta$ sont différents de 1, la spécification dépend de l’échelle. Un doublement des actifs et des emplois, répartis de la même façon, conduit à des flux qui ne sont pas proportionnels. Cette propriété est particulièrement difficile à comprendre (l'estimation dépend par exemple de l'unité choisie) mais s'avère très robuste à la procédure d'estimation par les moindres carrés.

Pour échapper à cette difficulté, et c'est le second problème, il faut changer la métrique d'estimation en utilisant une métrique qui permet de comparer deux distributions. Il existe plusieurs possibilités et la métrique de l'entropie relative fonctionne bien (voir l'encadré). La principale différence avec la métrique des moindres carrés ordinaires est que le poids donné aux erreurs du modèle n'est pas du tout le même suivant l'importance des flux. Une grande erreur relative sur un flux très petit ne doit pas avoir beaucoup d'importance alors que pour un flux très important, elle doit être très pénalisée. Les moindres carrés ordinaires donnent à chaque $\varepsilon_{ij}$, qui sont des erreurs relatives du fait de la forme en $log$, le même poids. Au contraire l'entropie relative pondère les erreurs par le terme $f_{ij}$. Cette métrique est d'autant plus importante que les données que l'on utilise sont très loin d'une distribution uniforme. Quelques paires origine-distribution concentrent la grande majorité des flux alors qu'un grand nombre de paires origine-destination représentent une part très faible des flux cumulés. Dans le cas de la Rochelle et de ses environs, les flux La Rochelle-La Rochelle pèsent presque 20% des flux totaux et les 40 flux les plus importants (sur 2 125) représentent plus de 50% de l'ensemble des flux.
...
MEAPS résoud chacun de ses problèmes explicitement, et en introduisant des paramètres dans la formulation de MEAPS on peut largement améliorer la qualité de l'ajustement.

Les estimations que nous présentons ici aboutissent à plusieurs résultats importants :

1.  une métrique pondérée produit des résultats plus robustes, plus convaincquant et une capacité de prédiction bien meilleure que la métrique implicite des moindres carrés ordinaires ;
...
3.  la modélisation des flux par MEAPS a de meilleures propriétés et une plus grande capacité prédictive que le modèle gravitaire, y compris lorsque celui ci est estimé en utilisant une métrique adaptée et une information géographique fine.
...
## Ajustements "communal" de modèles gravitaires et de *MEAPS* {#sec-ajustcom}
...
1.  La distance euclidienne entre les centroides de chaque commune, notée $euc$. Lorsque la destination est la même que l'origine, on utilise la moitié de la racine carré de la surface de la commune.
