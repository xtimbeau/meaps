...
L'estimation de modèle gravitaire est habituellement faite par une régression linéaire, par les moindres carrés ordinaires, où les flux observés sont la variable expliquée et les emplois, les actifs et la distance sont les facteurs explicatifs, de la forme suivante ($c$ est une constante, qui n'est pas estimée et déterminée pour respecter la contrainte en ligne) :

$$
log(f_{ij}) = \alpha \times log(a_i) + \beta \times log(e_j) + \delta \times d + c + \varepsilon_{ij}
$$ {#eq-gravitaire}
...
MEAPS résout chacun de ses problèmes explicitement, et en introduisant des paramètres dans la formulation de MEAPS (voir @sec-odds), on peut largement améliorer la qualité de l'ajustement.
