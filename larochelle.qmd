...
Le modèle formulé en effets fixes ou aléatoires est une formutation très particulère du modèle gravitaire. Les $a_i$ et $b_j$ représentent pour chaque origine $i$ ou chaque destination $j$ l'équivalent d'une masse d'actifs ou d'emplois, spécifique à l'origine ou à la destination, qui "remplace" ou complète la masse effectivement présente. En appliquant la procédure de Furness, on pose une hypothèse différente : l'élasticité à la masse d'actifs ou d'emplois est unitaire et les coefficients $a_i$ et $b_j$ permettent l'ajustement aux flux observés pour les marges.
...
## Gaussien ou multinomial

La stratégie d'estimation par les moindres carrés ordinaires (MCO) habituellement utilisée [@josselin2020, @lenormand2016] suppose que le processus générateur du bruit est gaussien. Cette hypothèse est forte. Une alternative est de considérer le tableau $ij$ des flux comme une table de contingence comptabilisant les flux dans chaque case. Au lieu d'une représentation implicitement continue, on privilègie une représentation discrète, et dans le cas où on ne conditionne pas les flux sur le total de la table ou les lignes ou les colonnes, on peut alors écrire, en suivant @flowerdew1982, que :

$$
P(f_{ij}=k) = \frac{e^{-\lambda_{ij}} \lambda_{ij}}{k!}
$$

La matrice de paramètres de Poisson $[\lambda_{ij}]$ peut alors être estimée à partir d'un modèle log-linéaire, qui peut être implémenté facilement par la procédure `glm`. On aura, en reprenant les notations employées :

$$
log(\lambda_{ij}) = \alpha \times log(n_i) + \beta \times log(e_j) - \delta \times d_{ij} + log(a_i)
$$

Un des apports de cette approche est que la variance estimée par ce modèle des erreurs n'est pas log-normale. Or le modèle log-normal (par les MCO) donne la même signification (le même poids) à une erreur de 100% pour un flux très petit qu'à une erreur de 100% pour un flux très grand. La log-vraisemblance pour le modèle log-lineaire s'écrit ($f_{ij}$ sont observés, $\hat f_{ij}$ estimés) :

$$
\mathcal{L}_{MCO} = - \frac{N}{2}log(2\pi\sigma^2) - \frac 1 {2 \sigma^2} \sum (log(f_{ij}) - log(\hat f_{ij}))^2 
$$

Elle est directement reliée à l'erreur quadratique moyenne. Pour le modèle discret de Poisson, la log-vraisemblance s'écrit :

$$
\mathcal{L}_{Poisson} = 2 \sum_{ij} f_{ij} log(f_{ij}/{\hat f_{ij}}) - \sum_{ij} (f_{ij} - \bar f)
$$

Cette expression est équivalente (pour la maximiser) au critère d'information employé dasn @lenormand2016 :

$$
I(t, \hat t) = \frac 1 N \sum_i {t_i \times (log(t_i) - log(\hat t_i))}
$$ {#eq-infcrit}

Cette métrique est aussi appelée l'entropie relative de deux distributions utilisée pour comparer deux distributions. Contraitrement à l'erreur quadratique moyenne, en pondérant par le flux le log de l'erreur, on donne aux flux importants un poids plus grand. On peut construire un critère proche du $R^2$ (voir @tip-entropie). La différence entre les deux métriques (MCO versus Poisson) sera d'autant plus importante que les données que l'on utilise sont très loin d'une distribution uniforme ou normale, ce qui est le cas pour les données de flux. Comme on le voit plus bas, c'est donc déterminant pour les résultats empiriques.
