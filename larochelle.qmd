...
$$
log(f_{ij}) = \alpha \times log(n_i) + \beta \times log(e_j) - \delta \times d_{ij} + log(a_i) + log(b_j) + \varepsilon_{ij}
$$ {#eq-gravdc} $$
...
En notant $\hat a_i = a_i/n_i^{1-\alpha}$ et $\hat b_j= b_j/e_j^{1-\beta}$ on a :
...
Ceci montre que le modèle gravitaire sous la double contrainte (par la procédure de Furness) est additif, puisque l'élasticité de chaque flux $f_{ij}$ aux emplois ou aux résidents est unitaire.
