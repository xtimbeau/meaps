...
Le cinquième et dernier problème est celui de la paramétrisation du modèle radiatif. @simini2012 proposent un paramètre supplémentaire à leur modèle en paramétrisant l'absorption à l'origine. @yang2014 introduisent un paramètre dans le modèle radiatif dans la formulation de @simini2012 par la loi élémentaire d'absorption. Sans surprise, l'introduction d'un paramètre supplémentaire améliore la qualité d'estimation et rapproche l'estimation .
...
3.  la modélisation des flux par MEAPS a de meilleures propriétés et une plus grande capacité prédictive que le modèle gravitaire, y compris lorsque celui ci est estimé en utilisant une métrique adaptée et une information géographique fine, à partir du moment où l'on introduit des paramètres dans le modèle radiatif. Le modèle radiatif universel (sans paramètre) produit un résultat correct, mais inférieur à ceux de modèles paramétrisés.

4.  Les paramètres du modèle gravitaire sont difficilement réconciliables avec l'interprétation. Ils dépendent en théorie de la configuration spatiale et de l'échelle d'observation. MEAPS permet une approche plus structurelle qui donne aux paramètres une signification plus ffacilkement interprétable.
...
Les distances entre chaque paire de cases permettent de calculer un indicateur d'accessibilité qui joue un rôle central dans le modèle radiatif, et donc dans *MEAPS*, en remplaçant la distance par la somme des opportunités en deçà d'un seuil de temps. Les cartes des `r acc_cr` représentent les temps pour accéder à un seuil d'emplois en utilisant différents modes de transport.
...
```{r}
#| label: tbl-gravcom
#| tbl-cap: Estimation commune à commune de modèles gravitaires non contraints
library(gtsummary)
