...
L'estimation de modèle gravitaire est habituellement faite par une régression linéaire, par les moindres carrés ordinaires, où les flux observés sont la variable expliquée et les emplois, les actifs et la distance sont les facteurs explicatifs, de la forme suivante :

$$
log(f_{ij}) = \alpha \times log(a_i) + \beta \times log(e_j) + \delta \times d + \varepsilon_{ij}
$$ {#eq-gravitaire}

Dans la lignée de [@lenormand2016], [@josselin2020] utilisent une forme similaire, avec quelques variations, pour estimer pour la région PACA, à partir des données MOBPRO, des modèles gravitaires. Ce sont les mêmes données que nous utilisons, dans une région différente, mais pour laquelle il est intéressant de comparer les approches. Différentes spécifications sont employées par @josselin2020, mais ce sont surtout des variations quant à la mesure de la distance ou de la population. Les modèles employés y sont avant tout gravitaires et utilisent la conclusion de @lenormand2016 qui est que le modèle gravitaire semble dominer les autres formalisations.

Cependant le modèle gravitaire et la stratégie d'estimation posent plusieurs problèmes : le premier est que lorsque $\alpha$ et $\beta$ sont différents de 1, la spécification dépend de l’échelle et n'est pas additive (voir supra). Cette propriété est particulièrement difficile à comprendre. Ainsi, l'estimation dépend par exemple de l'unité de découpage géographique choisie. Le second problème réside dans la métrioque d'estimation choisie. Dans @josselin2020, la seul métrique considérée est celle des erreurs moyennes au carré, pour une formulation en $log-log$. Ceci revient à minimiser le critère suivant :

$$
msre = \sum_i({log(\hat t}_i) - log(t_i))^2
$$
...
Cette dernière métrique est aussi appelée l'entropie relative de deux distributions et est le critère utilisé pour comparer deux distributions. Ce n'est pas tout à fait une distance (non symétrie, non nécessairement positif) mais elle peut être utilisée pour construire un critère proche du $R^2$ (voir @tip-entropie). La principale différence avec la métrique usuelle des moindres carrés ordinaires simple est que le poids donné aux erreurs du modèle n'est pas du tout le même suivant l'importance des flux. Une grande erreur relative sur un flux très petit ne doit pas avoir beaucoup d'importance alors que pour un flux très important, elle doit être très pénalisée. Or ce que l'on cherche à prévoir ce sont les flux et donc l'accent doit être mis sur la minimisation des erreurs sur les flux agrégés importants.

Les moindres carrés ordinaires donnent à chaque $\varepsilon_{ij}$, le même poids. Au contraire l'entropie relative pondère les erreurs par le terme $f_{ij}$. Cette métrique est d'autant plus importante que les données que l'on utilise sont très loin d'une distribution uniforme. Quelques paires origine-distribution concentrent la grande majorité des flux alors qu'un grand nombre de paires origine-destination représentent une part très faible des flux cumulés. Dans le cas de la Rochelle et de ses environs, les flux La Rochelle-La Rochelle pèsent presque 20% des flux totaux et les 40 flux les plus importants (sur 2 125) représentent plus de 50% de l'ensemble des flux. Une régression par les moindres carrés ordinaires est une approche alternative si l'on pondère par les flux la régression en $log-log$. Le critère employé est alors :

$$
msre = \sum_i { t_i \times ({log(\hat t}_i) - log(t_i))^2 }
$$
...
Le troisième problème est que l'on utilise généralement les distances à vol d'oiseau (ou euclidiennes) entre les unités administratives qui définissent la granularité d'observation. On s'expose à la fois au problème d'un découpage administratif et à une approximation des distances en faisant fi de la géographie fine. Nous calculons des distances en utilisant l'information sur les réseaux de transport (routiers ou transport en commun) entre un maillage bien plus fin que les unités administratives. Si on ne connaît pas les flux entre paires d'origine et de destination à un niveau plus fin que le niveau communal, on peut en revanche localiser les emplois ou les actifs (@sec-emplois) au carreau 200m et ainsi générer des distances entre les carreaux 200m ( @sec-distancesparmode). L'utilisation de ces informations supplémentaires se trouve être critique dans l'analyse des flux, améliorant l'estimation du modèle gravitaire.

Enfin, le quatrième problème, qui découle en partie de la non homogénéité du modèle, est que les flux doivent respecter une contrainte en ligne (chaque individu a un emploi et un seul) et en colonne (chaque emploi ne peut être occupé que par un individu). Or le respect de cette contrainte n'est pas garanti par l'@eq-gravitaire. L'usage est d'employer une procédure de Furness pour assurer le double équilibre en ligne et en colonne. Cependant, la justification théorique est incertaine et modifie les propriétés du modèle initial.

Le cinquième problème est celui de la paramétrisation du modèle radiatif. @simini2012 proposent un paramètre supplémentaire à leur modèle en paramétrisant l'absorption à l'origine. @yang2014 introduisent un paramètre dans le modèle radiatif dans la formulation de @simini2012 par la loi élémentaire d'absorption. Sans surprise, l'introduction d'un paramètre supplémentaire améliore la qualité d'estimation et la porte au dessus de ce que le modèle gravitaire peut faire. Le problème noté par @fotheringham1983 de variables manquantes dans le modèle gravitaire est ainsi résolu. Les variables "ajoutées" décrivent la structure spatiale, ou pour être plus précis la distribution jointe des résidents et des opportunités.

MEAPS résout chacun de ses problèmes explicitement, et en introduisant des paramètres dans la formulation de MEAPS on peut largement améliorer la qualité de l'ajustement.
...
4.  les paramètres du modèle gravitaire sont difficilement interpétables. Ils dépendent en effet de la configuration spatiale spécifique et de l'échelle d'observation. MEAPS permet une approche structurelle aux fondements bien définis qui donne aux paramètres une signification plus générale.
...
Les estimations que nous présentons dans cette section sont réalisées à la maille communale de MOBPRO, par une approche comparable à celle de @josselin2020, sur le périmètre de La Rochelle. Nous estimons dans un premier temps une famille de modèles gravitaires par les moindres carrés ordinaires. La forme estimée est celle de l'@eq-gravitaire en utilisant différentes distances.
