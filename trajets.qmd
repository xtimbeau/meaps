...
Comme nous connaissons la composition sociale de chaque carreau de résidents, le nombre de kilomètres parcourus par un individu moyen résident en *i* et travaillant en *j*, noté $km^m_{ij}$, s'obtient simplement en calculant la moyenne pondérée sur les caractéristiques des résidents en *i* de @eq-kmmod. En notant $prop_{ik}$ la proportion de la catégorie k au sein du carreau $i$, on a :
...
En effet, la stratégie retenue dans la littérature revue à la @sec-densemis consiste à estimer pour chaque individu observé le lien entre les kilomètres parcourus et les caractéristiques $X_k$ de l'individu, les caractéristiques $H_i$ de l'endroit $i(k)$ où cet individu $k$ vit et les caractéristiques $E_j$ de l'endroit $j(k)$ où il travaille (voir [@blaudindethé] pour une formulation récente) :
...
Comme on peut le constater au @tbl-empkm, entre une "commune très dense" et une "commune peu dense", il y a un ratio de un peu moins que 1 à 2 pour les kilomètres parcourus annuellement, pour le motif principal travail.
...
bd_read("distribution_nb_boucles") |> 
   rootogram(type = "hanging", 
                  ...
                  main_gp = gpar(fontsize = 15),
                                   main = "Distribution de la fréquence des boucles pour les actifs, hors Ile-de-France",
...
Comme on peut le constater au @tbl-logit-simple et comme on pouvait s'y attendre, les ménages avec enfants effectuent plus souvent des boucles complexes.
...
On peut voir au @tbl-rum-tc les estimations du modèle dans le cas où il y a des transports en commun. Le modèle à un $R^2$ de McFadden de `r mfr2`. Le modèle sans la possibilité de transports en commun n'est pas très différent.

<!--# graph des probas des modes selon la distance -->
