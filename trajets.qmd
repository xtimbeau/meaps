...
```{r} #| label: tbl-empkm #| tbl-cap: Nombre de kilomètres parcourus par an selon EMP2019 co2_emp <- bd_read("co2_emp_horsidf") co2_emp$table[[1]] |>   gt::tab_options(table.font.size = 12)}
```
...
La courbe en gris agrège les différents carreaux en pondérant par la population de chaque carreau et illustre le lien entre kilomètre et niveau de vie. Le nombre de kilomètres est le plus bas pour les ménages les plus pauvres – la contrainte budgétaire les oblige certainement – et les ménages les plus aisés. Pour un niveau de vie donné, la dispersion est très grande, mais le lien apparaît significatif pour autant. Les classes moyennes (ni les plus riches, ni les plus pauvres).
