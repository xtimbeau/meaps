...
On propose d'utiliser une autre analogie : le modèle radiatif de [@simini2012] ou des "*intervening opportunities*" de [@stouffer1940]
...
Si on a une mesure de la distance et que l'on défini **l'accessibilité** $s_i(d)=\sum _{j/d_{i,j}<d}1$ on a au premier ordre ($k$ est le nombre d'emploi en \$c_d\$, $\mu=\frac{-log(p_f)}{J})$ :
...
Si la densité des opportunités (les emplois) est uniforme, on peut calculer $s_i(d)=r^2/\rho^2$ et trouver une forme "gravitaire" qui dépend de la distance.
...
Mais, il y a $I!$ ordres possibles ce qui est impossible à traiter.
...
Intuitivement, chaque individu est localisé n'importe où, la saturation dépend surtout de la coïncidence d'individus proches qui sont les premiers servis. Cette coïncidence est rare et donc quelques tirages conduisent à un résultat proche de tous les tirages.
...
## Quelques aspects informatiques {.smaller}
...
::: column-screen
```{=html}
<iframe id="meaps-shiny" src="https://ofce.shinyapps.io/rmeaps/" style="border: none; width: 100%; height: 1000px" frameborder="0"></iframe>
```
:::
...
Le $R^2_{KL}$ pour le modèle gravitaire est de 77,8%, Le $R^2_{KL}$ pour MEAPS c200 est de 88,4%. f
...
## Apprentissage ou estimation non paramétrique versus estimation paramètrique {.smaller}
...
## Estimations paramétriques : modèles {.smaller}
...
## Estimations paramètriques : résultats
...
## Estimations paramétriques : résultats (2) {.smaller}
...
-   d'autres opportunités comme les écoles (en cours) en modifiant les processus de recherche ou en enlevant la saturation
...
On pourrait utiliser des informations plus fines (traces numériques, relevés de traffic, enquêtes locales) pour un calage plus fin, à intégrer éventuellement avec la maille communale.

## Conclusion (3)

On peut produire :

-   un chiffre agrégé plus fiable que les estimations habituellement produite. L'évaluation sur MOBPRO a une résolution spatiale trop faible, on peut gagner en précision grâce à l'interpolation et l'utilisation d'une information sur la géographie
...
<br> package : `devtools::install_github("maxime2506/rmeaps")`
