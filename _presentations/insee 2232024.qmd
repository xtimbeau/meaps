...
On définit **l'accessibilité** comme : $$
s_i(d)=\sum _{j/d_{i,j}<d}1
$$
...
On part d'une analogie radiative plutôt que gravitaire : le milieu traversé compte [@stouffer194 ; @simini2012 ; @simini2013]
...
En utlisant l'EMP 2019, on compte par individu le nombre de boucles (avec la pondération on estime les fréquences par strate).
...
La source est l'EMP 2019. On n'utilse pas le recensement qui interroge sur le mode habituel \[on pourrait vérifier quand même\].
...
-   La localisation des emplois au careeau 200m (recensement fichier détail MOBPRO + fichiers fonciers en imputant au prorata des surfaces par NAF 5 secteurs par commune, en complétant par le RFP pour les surfaces professionelles publiques)

-   Les réseaux de transport pour calculer les distances et les temps de parcours par mode entre chaque paire

-   les flux inter communaux (recensement fichier détail MOBPRO) pour caler le modèle de flux infracommunal
...
Sur ces sources ouvertes, on a produit des outils en open source afin d'assurer la transparence et la reproductibilité. En particulier le package [maxime2506/rmeaps](https://github.com/maxime2506/rmeaps) fournit le code C++/OMP optimisé pour simuler MEAPS avec une haute performance.
...
Actuellement, nous travaillons sur la métropole d'Aix-Marseille-Provence. C'est un passage à l'échelle significatif. La matrice de flux de la Rochelle est une matrice 5000 $\times$ 5000 $\approx$ 25M. Celle de Marseille est de 26000 $\times$ 38000 $\approx$ 1G, soit 40 fois plus grande. On calcule par exemple les paires origines destination à une vitesse entre 5000 et 1000 paire/s/vCPU.
...
D'autres traces numériques peut éventuellement permettre d'affiner un ou plusieurs des modèles employés (notamment MEAPS) et de les confronter à des résolutions infracommunales.
