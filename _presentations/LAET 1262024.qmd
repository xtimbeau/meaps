---
title: "MEAPS: modèle ergodique à absorption et saturation"
subtitle: "*Séminaire LAET*"
...
knitr::include_graphics("access_plm_50k.png")
...
## Ce qui demande de localiser emplois et résidents et ce qui les relie
...
-   **Trajets** : GTFS, OSM et routeur (dodgr, R5)

```{r}
knitr::include_graphics("output/popemp.png")
```

## Du potentiel à l'effectif

Comment produire cette carte de CO~2~ imputé aux résidents pour leurs mobilités professionnelles quotidienne ?
...
## Par rapport au modèle à 4 étapes

On part de la description du modèle à 4 étapes de @patrickbonnel2001

-   L'étape 1 (trajets sortants et entrants) repose sur l'observation à la commune (MOBPRO)/imputée au carreau 200m. Le modèle de fréquence (modèle 3) y participe.

-   MEAPS génére les flux entre *o* et *d* : étape 2 – distribution mais...

-   les flux sont générés à partir des trajets le long des réseaux (temps de parcours, dénivelé, vitesses) ce qui correspond à l'étape 4 –spécification

-   ces trajets sont calculés pour chaque mode (une partie de l'étape 3 – choix modal)

-   le choix modal intervient dans cette version de façon orthogonal (étape 3 – choix modal) et sera intégré prochainement à la génération des flux (étape 2 et 4)
...
-   *Empiriquement*, il faut utiliser un modèle de Poisson ou une fonction objectif entropie relative de Kullback-Leibler (KL) pour estimer un modèle de flux (MEAPS ou gravitaire).
...
```{r}
pvalue_stars <- function(x) dplyr::case_when(
  x < 0.001 ~ "<sup>\u002a\u002a\u002a</sup>",
  x < 0.01 ~ "<sup>\u002a\u002a</sup>",
  x < 0.05 ~ "<sup>\u002a</sup>",
  x > 0.05 ~ "" )

fmt_coef <- function(p, sd, se=rep(NA, length(p)), df=100) {
  cont <- abs(sd/p)<0.0001
  cont[is.na(cont)] <- TRUE
  l1 <- ifelse(
    is.na(p), "\\-",
    ifelse(cont, 
           str_c(signif(p,2)), 
           str_c(signif(p,3), pvalue_stars(2*pt(p/sd, df, lower.tail = FALSE)))))
  l2 <- ifelse(is.na(p), "",        
               ifelse(is.na(se)|cont, "", 
                 str_c("<br><small><em>(", signif(se,2),")</em></small>")))
  l3 <- ifelse(is.na(p), "",        
               ifelse(cont, "<br><small>(cont.)</small>", 
                      str_c("<br><small>(", signif(sd,2),")</small>")))
  str_c(l1, l2, l3)
}

data.tbl_glm <- bd_read("grav.est.collapsed") |> 
  filter(effect!="random") |>
  mutate(p1 = fmt_coef(p1_m, alpha.se, p1_sd),
         p2 = fmt_coef(p2_m, beta.se, p2_sd),
         p3 = fmt_coef(p3_m, delta.se, p3_sd),
         k = n-df) |> 
  mutate(ligne = 1:n(),
         method = tolower(method)) |>
  select(ligne, nom, method, k, df, p3, p1, p2, 
         r2kl=r2kl_m, r2kli=r2kli_m, dev.expl = dev.expl_m) |> 
  filter(str_detect(nom, "Grav")) |> 
  filter(!str_detect(nom, "quasipoisson"))

data.tbl_glm  |> 
  gt() |> 
  cols_label(ligne = "", nom = "",
             k = "p", df = "DL",
             method = "méthode",
             r2kl = md("R^2^~KLu~"), r2kli = md("R^2^~KLi~"),
             dev.expl = md("R^2^~dev~"),
             p1 = md("\u03B1"), p2 = md("\u03B2"), p3 = md("\u03B4")) |> 
  fmt_markdown(columns = c(p1,p2,p3)) |> 
  fmt_percent(columns = c(r2kl,r2kli,dev.expl), decimals = 1) |> 
  sub_missing() |> 
  cols_align(columns = -nom, align = "center") |> 
  tab_style(locations = cells_body(rows = seq(1, nrow(data.tbl_glm)-1, by = 3)),
            style = cell_borders(sides="top")) |> 
  tab_spanner(label = "Modèle", col=c(method, k, df)) |> 
  tab_spanner(label = "Paramètres", col=c(p3, p1, p2)) |> 
  tab_spanner(label = "Métriques", col=c(r2kl, r2kli, dev.expl)) 
```
...
```{r}
bd_read("estmeaps") |> 
  select(id, nom, obj, nbp, dl, p1, p2, r2kl, r2kli) |> 
  ...
  cols_label(id = "", nom = "", obj="objectif", 
             ...
             p1 = md("p~1~"), p2 = md("p~2~") ) |> 
  tab_spanner(label = "Modèle", col=c(obj, nbp, dl)) |> 
  tab_spanner(label = "Paramètres", col=c(p1, p2)) |> 
  tab_spanner(label = "Métriques", col=c(r2kl, r2kli)) |> 
  fmt_markdown(c(p1, p2, obj)) |> 
  fmt_percent(columns = c(r2kl, r2kli), decimals = 1) |> 
  cols_align(columns = -c(id, nom), align = "center") 
```
...
En intégrant les informations infracommunales, en simulant MEAPS au carreau 200m, en agrégeant au niveau communal (o) communal (d) on reproduit MOBPRO et on compare à l'observation (par un KL). MEAPS est paramétrisé (1, 2 ou 3 paramètres) et donc on peut déterminer les paramètres qui minimisent le KL comparant la distribution simulée et observée. On pourrait procéder à différentes agrégations pour reproduire plus d'"observées" avec les mêmes paramètres ou en ajoutant des paramètres supplémentaires.
...
![](images/clipboard-1134119035.png){fig-align="center"}
...
![](images/clipboard-3782242967.png){fig-align="center"}

## Lien revenu émissions à Marseille (IRIS)

![](images/clipboard-3150985983.png){fig-align="center"}

## Scénarios de densification : le lien entre la densité et la forme urbaine

![](images/clipboard-1842678492.png){fig-align="center"}
...
![](images/clipboard-1789958182.png){fig-align="center"}
...
-   décliner MEAPS sur une matrice $o\times d \times mode$ en les classant selon un coût généralisé, pour imputer les flux par mode. L'estimation sera alors faite sur 2 critères : flux MOBPRO plus EMP pour les choix modaux (ou MOBPRO modal)
...
Actuellement, nous travaillons sur la métropole d'Aix-Marseille-Provence. C'est un passage à l'échelle significatif : La matrice de flux de la Rochelle est 5000 $\times$ 5000 $\approx$ 25M. Celle de Marseille est de 26000 $\times$ 38000 $\approx$ 1G, soit 40 fois plus grande. On calcule par exemple les paires origines destination à une vitesse de l'ordre de 5000 paires/s/vCPU.

![](images/clipboard-376207725.png)
...
-   L'utilisation d'autres données comme celles de @lévy2023 (bornage de téléphone, qui permettent des matrices o/d à la résolution de l'IRIS). Un projet ANR est en cours (de candidature) avec un consortium (incluant le LAET) sur cette question (*Digitalisation et décarbonation des mobilités - MOBIDEC*). La future chaire Dynamiques Urbaines a pour objet d'accueillir ces travaux et de centraliser les ressources (dont les données de mobilité).

-   La génération des flux *o/d* permet de produire des flux par tronçon qui pourraient être ajustés sur des comptages observés.
