---
title: "Mobilités et Territoires"
subtitle: Projet pour une chaire de recherche en *topokynésie*
author: 
  - name: "Xavier Timbeau" 
...
format: 
  note-html: default
  note-pdf:
    output-file: Projet ofce chaire mobilités et territoires 2026
date: 2025-12-10
...
Nous voulons mettre à disposition du plus grand nombre une analyse fine — spatialement, socialement, selon les motifs — des mobilités quotidiennes dans les territoires permettant, entre autres, d'en faire le bilan carbone et de planifier l'aménagement des territoires que ce soit les transports ou l'identification des besoins en logements.

Pour cela, dans un premier temps, nous proposons d'étendre les analyses conduites à La Rochelle [@meaps2024b] ou Aix-Marseille-Provence aux principales agglomérations françaises et leurs aires urbaines fonctionnelles – une notion proche du bassin d'emploi. Dans un second temps, nous voulons nourrir ces analyses de données nouvelles, issues des opérateurs de téléphonie, afin de renforcer nos évaluations antérieures et de pouvoir les détailler dans certaines dimensions — comme la chrono-analyse. Dans un troisième temps, une catégorisation des formes urbaines et de leurs périphéries sera proposée, avec d'autres applications, pour valoriser les données produites. L'ensemble des contenus est produit en *science ouverte* et les données seront diffusées en *source ouverte*. La transparence et la reproductibilité renforceront la confiance dans les analyses produites afin que l'ensemble des acteurs s'en empare et que ces données puissent servir de référence.

Le projet est structuré comme une chaire de recherche à Sciences Po, au sein de l'OFCE — un laboratoire de Sciences Po. Il est articulé pour une séquence initiale de 3 ans pour un budget un peu supérieur à 500k€. Nous sommes à la recherche de partenaires pour le financer. Le financement est éligible au dispositif de mécénat d'entreprise.

Nous présentons ici le contexte (@sec-contexte), le détail du programme de recherche (@sec-recherche) ainsi que le calendrier (@sec-calendrier), le budget (@sec-budget) et les éléments d'organisation du projet (@sec-gouvernance).

*Ce document est confidentiel, merci de ne pas le diffuser.*

## La *topokynésie* : de l'accessibilité au trajets {#sec-contexte}
...
Nous proposons de partir de cette approche, en étendant l'analyse du potentiel aux réalisations de mobilités. C'est une façon de passer d'une géographie à une *topokynésie*, une analyse des mouvements qui s'inscrivent dans les lieux. A la place du seul *résider* on s'attache à comprendre comment sont *habités* les territoires. Le projet est à la fois théorique et empirique. La modélisation n'a de sens que si elle se nourrit et se confronte à des données riches et complexes.

Nous avons développé une modélisation qui ouvre la porte de cette *topokynésie.* La première étape est de calculer l'accessibilité à une échelle géographique fine (carreau INSPIRE 200m\times 200m), pour de nombreuses catégories d'opportunités (emploi, commerces, etc...) et pour différents modes de transport (à pied, en transport en commun, en vélo ou en voiture). La résolution retenue autorise de prendre en compte de nombreuses nuances dans les réseaux de transport — jusqu'à la localisation d'un arrêt de bus — ou la localisation des aménités. Cela permet une meilleure compréhension des territoires dès que cette analyse peut être systématisée.

![](images/clipboard-3675797071.png)

*Ces cartes d’accessibilité à une résolution 200m⨉200m sont produites par l'analyse de tous les trajets possibles entre les lieux de résidence et les opportunités d'emploi*
...
*Cette carte illustre la changement d'accessibilité entre le système actuel de transport et celui projeté en 2030 avec la mise en service complète du Grand Paris Express. Les zones sont colorées en fonction de la réduction de temps apportée pour l'accès à au moins 50k emplois.*

La seconde étape est de passer du potentiel ainsi quantifié aux flux effectifs. Pour cela, nous avons développé un modèle théorique [@meaps2023] qui est une alternative au modèle gravitaire habituellement utilisé dans les modélisations des transports. Il affecte des flux de mobilité vers des aménités ou des opportunités — c'est-à-dire des lieux d'intérêt — en fonction de distances ou de coûts généralisés, en permettant de prendre en compte la double concurrence des individus voisins pour les mêmes opportunités (lorsqu'elles ont une capacité finie) et, réciproquement, la concurrence des opportunités pour les individus (lorsque les opportunités doivent être remplies, e.g. des emplois). Associées à cette modélisation, nous avons développé des méthodes d'estimation qui permettent d'ajuster le modèle sur des données comportementales comme l'enquête mobilité des personnes [@meaps2024a]. L'approche empirique permet de joindre des sources de données disparates (par exemple d'un côté, des flux agrégés sur une année et pour l'ensemble des ménages, mais détaillés sur une matrice orgine destination, de l'autre des données d'enquêtes sur un échantillon de ménages identifiant leur comportement de mobilité suivant les motifs ou leurs caractéristiques).
...
|  |  |
|:----------------------------------:|------------------------------------|
| ![](images/clipboard-362203475.png){width="300" fig-align="center"} | ![](images/clipboard-1456870736.png){width="300" fig-align="center"} |
...
[^1]: Les 50 premières agglomérations françaises vont de Paris à Troyes (La Rochelle est 49^eme^). Ces agglomérations incluent des agglomérations frontalières, avec des navettages importants vers des pays étrangers (Lille-Roubaix, Annecy, Strasbourg, Valenciennes, Metz, Bayonne entre autres) qui posent des problèmes spécifiques.
...
## Calendrier prospectif et livrables {#sec-calendrier}
...
## Budget du projet {#sec-budget}

Le projet est une chaire de recherche, hébergée à l'OFCE, dont la phase initiale de financement s'ouvre pour 3 ans.

Le budget de la chaire est détaillé dans le tableau suivant. Il est principalement est composé du coût d'un chercheur à temps plein et d'un assistant de recherche à temps partiel pour 3 ans. Un chercheur additionnel représente, suivant le degré de séniorité un coût brut chargé entre 80k€ par an. Un assistant de recherche, parce qu'à temps partiel, coûte par an autour de 30k€.

A cela s'ajoutent un budget réalisation du site web (50k€, pour les 3 ans), un budget informatique de calcul (10k€ par an), l'encadrement de la chaire (30k€), les frais de structure (5%) et les frais de gestion (15%) hors taxes. Le coût moyen annuel est d'un peu moins de 200k€ (192k€).

La chaire bénéficie du régime fiscal du mécénat d'entreprise, autorisant les partenaires financeurs soumis à l'Impôt sur les Sociétés à une réduction d'impôt de 60% des sommes engagées, dans la limite de 2 millions d'euros (40% au delà) et plafonné à 20k€ ou 0,5% du chiffre d'affaire de l'entreprise.

```{r, include=FALSE}
library(tidyverse)
library(gt)

data <- readxl::read_excel("budget mobilités et territoires.xlsx", sheet = "budget")
```
...
data |> 
  select(1:5) |> 
  slice(1:9) |>
  mutate(across(-Dépenses, as.numeric)) |> 
  gt() |> 
  tab_options(table.font.size = 11) |> 
  fmt_number(where(is.numeric), scale=1/1000, sep_mark = " ", dec_mark = ",", pattern = "{x}k€", decimals =0) |> 
  tab_style(cell_text(indent = px(6)), locations = cells_body(columns = Dépenses, rows = !Dépenses |> str_detect("Total"))) |> 
  tab_style(cell_text(weight = "bold"), locations = cells_body(rows = Dépenses |> str_detect("Total"))) |> 
  cols_label(`total dépenses` = "Années 1 à 3") |> 
  cols_width(
    Dépenses ~ px(200),
    everything() ~ px(80))
...
Le ticket d'entrée moyen est donc de 40k€ par an avant le levier fiscal, et, en incluant le levier fiscal, il est de 13,3k€ par an ou encore de 40k€ sur les trois ans.

## Propriété intellectuelle & gouvernance {#sec-gouvernance}

Le financement de la chaire impose que les partenaires financeurs ne reçoivent aucune contrepartie directe qui pourrait être associé à une prestation — ce qui conduirait à requalifier les productions de la chaire en prestations.
