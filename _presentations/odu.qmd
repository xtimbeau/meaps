...
subtitle: Projet pour une chaire de recherche en *topkynésie*
author: 
  - name: "Xavier Timbeau" 
...
## La *topokynésie* : de l'accessibilité au trajets,
...
Nous avons développé une modélisation qui ouvre la porte de cette *topokynésie.* La première étape est de calculer l'accessibilité à une échelle géographique fine (carreau INSPIRE 200m\$\times\$200m), pour de nombreuses catégories d'opportunités (emploi, commerces, etc...) et pour différents modes de transport (à pied, en transport en commun, en vélo ou en voiture). La résolution retenue autorise de prendre en compte de nombreuses nuances dans les réseaux de transport ou la localisation des aménités. Cela permet une meilleure compréhension des territoires dès que cette analyse peut être systématisée.
...
```{r}
library(tidyverse)
readxl::read_excel("budget mobilités et territoires.xlsx", sheet = "budget") |> 
  select(1:5) |> 
  slice(1:6) |> 
  gt::gt()
```
